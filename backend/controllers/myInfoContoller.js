...
const createPersonal = async (req, res) => {
  const { firstName, lastName, nic, nationality, maritalStatus, dob, gender } =
    req.body;
  ...
  try {
    if (!firstName || !nic || !nationality || !dob) {
      return res
        .status(400)
        .json({ success: false, message: "Other Fields are Required" });
    }
...
const createEmergency = async (req, res) => {
  ...
  try {
    ...
    const emergency = new EmergencyDetail({
      name,
      relationship,
      mobile,
      employeeId,
...
const deleteEmergency = async (req, res) => {
  const { _id } = req.params;
  try {
    const deletedEmergency = await EmergencyDetail.findByIdAndDelete({ _id });
    if (!deletedEmergency) {
      return res.status(404).json({
        success: false,
        message: "Emergency details not found",
...
const updateJob = async (req, res) => {
  ...
  if (jobSpecification !== undefined)
    updateData.jobSpecification = jobSpecification;
  ...
  try {
    const updatedJob = await JobDetail.findByIdAndUpdate(_id, updateData, {
      new: true,
      runValidators: true,
    });
...
const deleteJob = async (req, res) => {
  const { _id } = req.params;
  try {
    const deletedJob = await JobDetail.findByIdAndDelete({ _id });
    if (!deletedJob) {
      return res.status(404).json({
        success: false,
